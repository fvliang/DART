...
def format_text_with_highlight(token_ids, tokenizer):
    """
    将 token_ids 格式化为带高亮的 HTML 文本
    第一个 token 是原模型产生的，从第二个开始是 draft model 产生的（需要高亮）
    
    Args:
        token_ids: torch.Tensor, 新生成的 token IDs
    ...
    # 从第二个 token 开始是 draft model 产生的
    ...
    # 构建带高亮的 HTML 文本
    # 使用 <mark> 标签高亮显示 draft model 产生的 token
    formatted_html = (
...
def build_demo():
    ...
    with gr.Blocks(css=custom_css, theme=gr.themes.Base(
        ...
        # 标题区域
        gr.Markdown("# 🚀 DART Chatbot", elem_id="main-title")
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1, min_width=320):
                with gr.Group(elem_id="control-panel"):
                    gr.Markdown("### 🎯 Model Selection", elem_classes="panel-title")
                    ...
                    gr.Markdown("### 🎛️ Generation Settings", elem_classes="panel-title")
                    ...
                    gr.Markdown(
                        "💡 **Compression Ratio** = Generated Tokens ÷ Forward Passes. Higher is better!",
            ...
            with gr.Column(scale=3, min_width=600, elem_id="chat-column"):
                ...
                with gr.Group():
                    ...
                    with gr.Row():
                        send_button = gr.Button("🚀 Send", variant="primary", elem_classes="primary-btn", scale=2)
                        regenerate_button = gr.Button("🔄 Regenerate", elem_classes="secondary-btn", scale=1)
                        stop_button = gr.Button("⏹️ Stop", elem_classes="danger-btn", scale=1)
                        clear_button = gr.Button("🗑️ Clear", elem_classes="secondary-btn", scale=1)
