...
def update_inference_inputs(
    ...
    draft_tokens, retrieve_indices,tree_mask,tree_position_ids,draft_forward_t = model.ea_layer.topK_genrate(accept_hidden_state_new,
                                              input_ids=torch.cat((input_ids, token.to(input_ids.device)), dim=1),
                                              head=model.base_model.lm_head,logits_processor=logits_processor,log=True)


    new_token += accept_length + 1

    return input_ids, draft_tokens, retrieve_indices,tree_mask,tree_position_ids, new_token, None, token, draft_start_t, draft_forward_t
