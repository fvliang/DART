...
class Model(nn.Module):
    ...
    @torch.no_grad()
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor, log=False):
        ...
        if log:
            ...
            draft_forward_t_0 = time.time()  # [EVAL]
        ...
        if log:
            torch.cuda.synchronize()    # [EVAL]
            draft_forward_t_0 = time.time() - draft_forward_t_0  # [EVAL]
        ...
        # 4
        if log:
            draft_forward_t_1_total = 0  # [EVAL]
        for i in range(depth):
            ...
            if log:
                torch.cuda.synchronize()    # [EVAL]
                draft_forward_t_1 = time.time()  # [EVAL]
            out_hidden, past_key_values = self(input_hidden, input_ids=input_ids, past_key_values=past_key_values,
                                               position_ids=position_ids, use_cache=True)
            if log:
                torch.cuda.synchronize()    # [EVAL]
                draft_forward_t_1 = time.time() - draft_forward_t_1  # [EVAL]
                draft_forward_t_1_total += draft_forward_t_1  # [EVAL]
            len_posi += 1
        ...
        if log:
            return draft_tokens, retrieve_indices, tree_mask, tree_position_ids, draft_forward_t_0 + draft_forward_t_1_total
